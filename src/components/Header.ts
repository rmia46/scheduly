...
export function renderHeader(container: HTMLElement): void {
  ...
  container.innerHTML = `
    <header class="bg-white/80 border-b border-slate-200/80 sticky top-0 z-40 px-4 py-2.5 flex items-center justify-between shadow-xs transition-colors duration-200">
      <!-- Left: Sidebar toggle + App Branding -->
      <div class="flex items-center gap-3">
        <button id="hdr-toggle-sidebar" class="w-9 h-9 rounded-xl flex items-center justify-center text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors cursor-pointer" title="Toggle Sidebar">
