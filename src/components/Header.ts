  ...
  container.innerHTML = `
    <header class="bg-white/95 border-b sticky top-0 z-40 px-3 sm:px-4 py-2 flex items-center justify-between shadow-xs transition-all duration-300" style="border-color: ${theme.border};">
      ...
      <div class="flex items-center gap-2 sm:gap-3">
        <!-- Mode / Panel Toggle (Editor Mode vs Full View Mode) -->
        <button id="hdr-toggle-sidebar" class="h-8 px-2.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer border text-xs font-medium ${
          state.sidebarOpen
            ? 'border-transparent shadow-2xs font-semibold'
            : 'border-slate-200/90 text-slate-600 hover:text-slate-900 hover:bg-slate-100 bg-white/80'
        }" style="${
          state.sidebarOpen
            ? `background-color: ${theme.badgeBg}; color: ${theme.accent}; border-color: ${theme.border};`
            : ''
        }" title="${state.sidebarOpen ? 'Switch to Full View (⌘B / Ctrl+B)' : 'Open Editor Panel (⌘B / Ctrl+B)'}">
          <svg class="w-4 h-4 shrink-0 transition-transform ${state.sidebarOpen ? '' : 'text-slate-500'}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
            <line x1="9" y1="3" x2="9" y2="21"></line>
          </svg>
          <span class="hidden sm:inline text-[11px] leading-none">${state.sidebarOpen ? 'Editor' : 'Full View'}</span>
          <kbd class="hidden md:inline-block text-[9px] px-1 py-0.2 rounded border font-mono font-normal opacity-65 ${
            state.sidebarOpen ? 'border-current/30' : 'border-slate-200 bg-slate-50 text-slate-400'
          }">⌘B</kbd>
        </button>
